...
import com.example.stpos.databinding.FragmentAddFundBinding;
...
public class AddFundFragment extends Fragment {
    ...
    public FragmentAddFundBinding binding;
    ...
    public static AddFundFragment newInstance(String param1, String param2) {
        AddFundFragment fragment = new AddFundFragment();
    ...
    private void datavalidation() {
        if (!TextUtils.isEmpty(binding.addFund.getText().toString())) {
            datasubmit();
        } else {
            binding.addFund.setError("Please fill out this field");
            binding.addFund.requestFocus();
        }
    }
    ...
    private void datasubmit() {
        ...
        progressDialog.show();
        api.addfunds(binding.addFund.getText().toString()).enqueue(new Callback<JsonObject>() {
            @Override
            public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                progressDialog.dismiss();
                if (response.isSuccessful() && response.body() != null){
                    Navigation.findNavController(getView()).navigate(R.id.action_global_fundManageFragment);
                    Toast.makeText(getContext(), "Add Fund Successfully", Toast.LENGTH_SHORT).show();
                }
            }
